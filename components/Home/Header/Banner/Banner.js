                        ...
                        return <div key={index}>
                            <div className="flex justify-center" style={{background: "linear-gradient(0deg, rgba(35, 31, 32, 0.5), rgba(35, 31, 32, 0.5))"}}>
                                ...
                                <div className="absolute top-0 text-center w-[9%]">
                                    <h1 className="text-white text-4xl mt-[8%] mb-[3%] md:text-5xl font-semibold md:mt-[11%]">{data.title}</h1>
                                    <hr className='mb-[7%] border-t-2' />
                                    <span className="duration-200 text-white cursor-pointer border px-4 py-1 bg-gradient-to-r from-gray-600 to-gray-600 bg-blend-overlay md:text-xl md:px-12 md:py-2 md:font-semibold hover:bg-gray-500/40">
                                        <Link href={data.link}>{data.button}</Link></span>
                                    <p className="text-white mt-[20%] sm:mt-[30%] md:mt-[35%] md:text-2xl text-sm sm:text-lg sm:font-semibold">{data.desc}</p>
                                </div>
                            </div>
                        </div>
                    })
